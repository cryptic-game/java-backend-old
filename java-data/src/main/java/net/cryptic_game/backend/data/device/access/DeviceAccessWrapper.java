...
public class DeviceAccessWrapper {
    ...
    public static boolean hasUserAccessToDevice(final User user, final Device device) {
        try (Session sqlSession = sqlConnection.openSession()) {
            return sqlSession
                    .createQuery(
                            "select object (a) from DeviceAccess a where a.user = :user and a.device = :device and a.valid = true and a.expire > :currentDate",
                            DeviceAccess.class)
    ...
    public static DeviceAccess grantAccessToDevice(final User user, final Device device,
    ...
    public static List<DeviceAccess> getAccessesToDevice(final Device device) {
        try (Session sqlSession = sqlConnection.openSession()) {
            return sqlSession
                    .createQuery(
                            "select object (a) from DeviceAccess a where a.device = :device and a.valid = true and a.expire > :currentDate",
                            DeviceAccess.class)
                    .setParameter("device", device)
                    .setParameter("currentDate", LocalDateTime.now())
                    .getResultList();
