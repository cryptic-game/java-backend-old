...
import net.cryptic_game.backend.base.api.ApiEndpointExecutor;
...
import net.cryptic_game.backend.base.api.ApiParameterData;
import net.cryptic_game.backend.base.api.ApiParameterException;
import net.cryptic_game.backend.server.client.Client;

public class ServerApiEndpointExecutor extends ApiEndpointExecutor {

  public ServerApiEndpointExecutor(String name, ApiCollection apiCollection, Method method)
  ...
  protected List<ApiParameterData> loadParameters(Method method) throws ApiParameterException {
    ...
    if (method.getParameters()[0].getType() != Client.class) {
      throw new ApiParameterException(
          "First parameter \"" + method.getParameters()[0].getName() + "\" of endpoint \""
              + this.name + "\" is not \"" + Client.class.getName() + "\" type.");
    }
    ...
    for (int i = 1; i < method.getParameters().length; i++) {
      ...
      if (!parameter.isAnnotationPresent(ApiParameter.class)) {
        throw new ApiParameterException(
            "Parameter \"" + parameter.getName() + "\" of endpoint \"" + this.name
                + "\" does not have \"" + ApiParameter.class.getName() + "\" annotation.");
      }
      final ApiParameter apiParameter = parameter.getAnnotation(ApiParameter.class);
      parameters.add(new ApiParameterData(apiParameter.value().toLowerCase(), parameter.getType(),
          apiParameter.optional()));
    }
  ...
  public JsonObject execute(ApiExecutionData data) throws ApiException {
    if (!(data instanceof ServerApiExecutionData)) {
      throw new ApiException("Provided " + data.getClass().toString()
