...
public class HttpServerHandler extends NettyHandler<Object> {
    ...
    private FullHttpResponse build(final HttpResponseStatus status, final JsonObject json) {
        ...
        response.headers()
                .set(HttpHeaderNames.CONTENT_TYPE, "application/json; charset=" + StandardCharsets.UTF_8);

        return response;
    }

    private void write(final ChannelHandlerContext ctx, final FullHttpRequest request,
