...
public abstract class AppBootstrap {
    ...
    private void initSQLTableModels() throws SQLException {
        for (Class<? extends TableModel> modelClass : new Reflections(
                "net.cryptic_game.backend.base.sql").getSubTypesOf(TableModel.class)) {
    ...
    private void initSentry() {
        ...
        if (!dsn.isBlank()) {
            ...
            client.setEnvironment(
                    this.config.getAsBoolean(BaseConfig.PRODUCTIVE) ? "production" : "development");
...
//            client.addExtra();
    ...
    private void setLoglevel(final Level level) {
        ...
        final LoggerConfig loggerConfig = ctx.getConfiguration()
